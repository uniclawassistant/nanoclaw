...
// Reaction-triggers loader is hot-reloading per call. Tests override the
// returned set per case to assert filter behavior without touching disk.
const reactionTriggersRef = vi.hoisted(() => ({
  current: new Set<string>(['✅']),
}));
vi.mock('../reaction-triggers.js', () => ({
  loadReactionTriggers: () => reactionTriggersRef.current,
}));
...
vi.mock('grammy', () => ({
  Bot: class MockBot {
    ...
    startOpts: any = null;
    start(opts: { onStart: (botInfo: any) => void; allowed_updates?: any }) {
      this.startOpts = opts;
...
describe('TelegramChannel', () => {
  ...
  describe('message_reaction handling', () => {
      ...
      return {
        ...
        messageReaction: {
          ...
          user: args.anonymous
            ? undefined
            : {
                id: args.userId ?? 99001,
                first_name: args.userFirst ?? 'Fedor',
                username: args.userUsername,
              },
          date: args.date ?? 1_700_000_000,
        },
        me: { id: args.botId ?? 12345 },
      };
    }

    async function fireReaction(ctx: any) {
      const handlers =
    ...
    function makeOpts(): TelegramChannelOpts & {
      getReactionWakeContext: any;
    } {
      const base = createTestOpts();
      const lookup = vi.fn().mockReturnValue({
        myText: 'awaiting your call',
        priorUserMessageText: 'do the thing',
      });
      return {
        ...base,
        getReactionWakeContext: lookup,
      } as any;
    }

    beforeEach(() => {
      reactionTriggersRef.current = new Set(['✅']);
      vi.useFakeTimers();
    });
    ...
    it('passes message_reaction in allowed_updates when starting the bot', async () => {
      const channel = new TelegramChannel('test-token', makeOpts());
      await channel.connect();
      const startOpts = currentBot().startOpts;
      expect(startOpts).not.toBeNull();
      expect(startOpts.allowed_updates).toContain('message_reaction');
    });

    it('registers a message_reaction filter handler on connect', async () => {
      const channel = new TelegramChannel('test-token', makeOpts());
      await channel.connect();
      expect(currentBot().filterHandlers.has('message_reaction')).toBe(true);
    });

    it('wakes on whitelisted emoji reaction to a stored bot outbound', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await fireReaction(createReactionCtx({ new: ['✅'] }));
      await vi.advanceTimersByTimeAsync(2100);

      expect(opts.getReactionWakeContext).toHaveBeenCalledWith(
        'tg:100200300',
        '6701',
      );
      expect(opts.onMessage).toHaveBeenCalledTimes(1);
      ...
      expect(msg.sender_name).toBe('Fedor');
      expect(msg.is_from_me).toBe(false);

      // Body envelope: trigger prefix + <reaction>{...}</reaction> with the
      // FED-22 payload shape — neutral, no semantic labels.
      expect(msg.content).toContain('@Andy');
      const inner = msg.content.match(/<reaction>(.+)<\/reaction>/)?.[1];
      ...
      expect(payload).toEqual({
        ...
        emoji: '✅',
        message_id: '6701',
        my_text: 'awaiting your call',
        reactor: 'Fedor',
        prior_user_message_text: 'do the thing',
      });
    });

    it('does NOT wake on emoji outside whitelist', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).not.toHaveBeenCalled();
      expect(opts.getReactionWakeContext).not.toHaveBeenCalled();
    });

    it('does NOT wake when reactor is the bot itself', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await fireReaction(
        createReactionCtx({ new: ['✅'], userId: 12345, botId: 12345 }),
      );
      await vi.advanceTimersByTimeAsync(2100);
    ...
    it('does NOT wake when target message is not a stored bot outbound', async () => {
      const opts = makeOpts();
      (opts.getReactionWakeContext as any).mockReturnValueOnce(null);
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('does NOT wake when reactor is anonymous (actor_chat)', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await fireReaction(createReactionCtx({ new: ['✅'], anonymous: true }));
      await vi.advanceTimersByTimeAsync(2100);

      expect(opts.onMessage).not.toHaveBeenCalled();
    });

    it('debounces add → remove inside 2s settle window (no wake)', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await fireReaction(createReactionCtx({ old: [], new: ['✅'] }));
    ...
    it('wakes once on add → remove → add (final state with whitelist)', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('hot-reloads the whitelist between events', async () => {
      const opts = makeOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      // First event: 🔁 not yet whitelisted — no wake.
      await fireReaction(createReactionCtx({ new: ['🔁'] }));
      await vi.advanceTimersByTimeAsync(2100);
      expect(opts.onMessage).not.toHaveBeenCalled();

      // Add 🔁 to whitelist (no process restart).
      reactionTriggersRef.current = new Set(['✅', '🔁']);
    ...
    it('uses the prior user message text from the lookup', async () => {
      const opts = makeOpts();
      (opts.getReactionWakeContext as any).mockReturnValueOnce({
        myText: 'sent it',
        priorUserMessageText: null,
      });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
